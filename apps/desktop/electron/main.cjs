...
async function runAutomatedUat(window) {
  ...
  try {
    ...
    const submittedChat = await window.webContents.executeJavaScript(`(async () => { const prompt = document.querySelector('#conversation-composer-prompt') ?? document.querySelector('#chat-prompt'); if (!(prompt instanceof HTMLTextAreaElement) || prompt.disabled) return false; const setter = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, 'value')?.set; setter?.call(prompt, 'Confirm the preview relay boundary.'); prompt.dispatchEvent(new Event('input', { bubbles: true })); await new Promise((resolve) => setTimeout(resolve, 60)); const submit = prompt.form?.querySelector('[data-prompt-input-submit], button[type="submit"]'); if (!(submit instanceof HTMLButtonElement) || submit.disabled) return false; submit.click(); return true; })()`);
