...
export const startHarnessRun = command(StartRunInput, async (input) => {
  ...
  const worktree = (await repository.listWorktrees()).find((item) => item.worktreeId === input.worktree.worktreeId);
  ...
  const result = await getHermesServeRunManager().start(normalized, getActiveHermesClient().executionContext().connection, getActiveHermesClient().executionContext().token);
...
export const getHarnessRunEvents = query(runId.extend({ after: z.number().int().nonnegative().default(0) }), async ({ runId, after }) => {
  ...
  if (terminalStatus && ['completed', 'failed', 'cancelled'].includes(terminalStatus)) {
    const run = (await getCompanionRepository().listWorktrees()).find((item) => item.writerRunId === runId);
    if (run) {
...
export const cancelHarnessRun = command(runId, async ({ runId }) => {
  ...
  const worktree = (await getCompanionRepository().listWorktrees()).find((item) => item.writerRunId === runId);
