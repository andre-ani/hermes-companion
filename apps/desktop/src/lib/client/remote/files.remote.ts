...
export const listWorktreeFiles = query(target, async ({ worktreeId, path }) => {
  ...
  return invokeExecutionHost<FileEntry[]>({
    ...
    remoteCapability: 'files', remotePayload: { action: 'file.list', worktreeId, path }, expectedConnectionId: worktree.connectionId
  });
});
...
export const readWorktreeFile = query(target.extend({ path: z.string().min(1).max(4_096) }), async ({ worktreeId, path }) => {
  ...
  return invokeExecutionHost<{ path: string; content: string; size: number }>({
    ...
    remoteCapability: 'files', remotePayload: { action: 'file.read', worktreeId, path }, expectedConnectionId: worktree.connectionId
  });
});

export const previewWorktreeFile = query(target.extend({ path: z.string().min(1).max(4_096) }), async ({ worktreeId, path }) => {
  const worktree = await requireActiveWorktree(worktreeId);
  return invokeExecutionHost<FilePreview>({ localCapability: 'file.preview', localInput: { root: worktree.path, path }, remoteCapability: 'files', remotePayload: { action: 'file.preview', worktreeId, path }, expectedConnectionId: worktree.connectionId });
});
...
export const saveWorktreeFile = command(target.extend({ path: z.string().min(1).max(4_096), content: z.string().max(2_097_152) }), async ({ worktreeId, path, content }) => {
  ...
  const result = await invokeExecutionHost<{ path: string; size: number }>({
    ...
    remoteCapability: 'files', remotePayload: { action: 'file.write', worktreeId, path, content }, expectedConnectionId: worktree.connectionId
  });
...
export const searchWorktreeFiles = query(target.extend({ query: z.string().trim().min(1).max(500), limit: z.number().int().min(1).max(500).default(200) }), async ({ worktreeId, query: search, limit }) => {
  ...
  return invokeExecutionHost<FileSearchResult[]>({
    ...
    remoteCapability: 'files', remotePayload: { action: 'file.search', worktreeId, query: search, limit }, expectedConnectionId: worktree.connectionId
  });
});
...
export const createWorktreeFileEntry = command(target.extend({ path: z.string().min(1).max(4_096), kind: z.enum(['file', 'directory']) }), async ({ worktreeId, path, kind }) => {
  const worktree = await requireActiveWorktree(worktreeId); const result = await invokeExecutionHost<{ path: string; kind: 'file' | 'directory' }>({
    localCapability: 'file.create', localInput: { root: worktree.path, path, kind }, remoteCapability: 'files', remotePayload: { action: 'file.create', worktreeId, path, kind }, expectedConnectionId: worktree.connectionId
  });
...
export const moveWorktreeFileEntry = command(target.extend({ from: z.string().min(1).max(4_096), to: z.string().min(1).max(4_096) }), async ({ worktreeId, from, to }) => {
  const worktree = await requireActiveWorktree(worktreeId); const result = await invokeExecutionHost<{ from: string; to: string }>({
    localCapability: 'file.move', localInput: { root: worktree.path, from, to }, remoteCapability: 'files', remotePayload: { action: 'file.move', worktreeId, from, to }, expectedConnectionId: worktree.connectionId
  });
...
export const deleteWorktreeFileEntry = command(target.extend({ path: z.string().min(1).max(4_096), recursive: z.boolean().default(false) }), async ({ worktreeId, path, recursive }) => {
  const worktree = await requireActiveWorktree(worktreeId); const result = await invokeExecutionHost<{ path: string; kind: 'file' | 'directory' }>({
    localCapability: 'file.delete', localInput: { root: worktree.path, path, recursive }, remoteCapability: 'files', remotePayload: { action: 'file.delete', worktreeId, path, recursive }, expectedConnectionId: worktree.connectionId
  });
