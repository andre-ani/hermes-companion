...
import { z } from 'zod';
import { DesktopPreferences, GatewayConnection, PreviewLease, ProfileUiPreferences, ProjectBinding, workspaceLayoutOwnerKey, WorkspaceLayoutOwner, WorkspaceLayoutPreferences, WorktreeRecord } from '@hermes-companion/contracts';
...
const PreviewRecord = z.object({ id: z.string().uuid(), worktreeId: z.string(), origin: z.string().url(), relayUrl: z.string().url().nullable(), expiresAt: z.string().datetime() });
...
const initialState = (): CompanionState => ({
  ...
  projects: [], worktrees: [], previews: [], profileUi: {}, desktopPreferences: DesktopPreferences.parse({}), workspaceLayouts: {}, pinnedSessionIds: [], unreadSessionIds: [], audit: []
});
...
export class CompanionRepository {
  ...
  private async load() {
    ...
    try {
      ...
      this.state = CompanionState.parse(raw);
    } catch (error) {
  ...
  async addPreview(preview: z.infer<typeof PreviewLease>) {
    ...
    this.addAuditToState(state, 'preview.started', preview.id, { worktreeId: preview.worktreeId });
