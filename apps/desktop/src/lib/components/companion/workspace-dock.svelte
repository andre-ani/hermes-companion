<script lang="ts">
  ...
  import { ArrowLeft, ArrowRight, ArrowUpRight, Bot, ChevronLeft, Code2, File, FilePlus2, Folder, FolderGit2, FolderPlus, FolderTree, GitCompareArrows, Globe2, Maximize2, Pencil, Play, Plus, RotateCw, Save, Search, Terminal, Trash2, X } from '@lucide/svelte';
  ...
  let { worktree = null, gitWorkspace = null, unavailableReason = null, browserOwnerKey, browserLeaseId, visible, onchanged, onopenworktrees, onfullscreenchange, dockTab = $bindable('surfaces'), openTabs = $bindable<string[]>([]) }: { worktree?: WorktreeRecord | null; gitWorkspace?: HermesGitWorkspace | null; unavailableReason?: string | null; browserOwnerKey: string; browserLeaseId: string; visible: boolean; onchanged?: () => void | Promise<void>; onopenworktrees?: () => void; onfullscreenchange?: (fullscreen: boolean, identity: { ownerKey: string; browserLeaseId: string }) => void; dockTab?: string; openTabs?: string[] } = $props();
  ...
  const surfaceOptions = [{ id: 'files', label: 'Files', icon: File }, { id: 'terminal', label: 'Terminal', icon: Terminal }, { id: 'browser', label: 'Browser', icon: Globe2 }, { id: 'changes', label: 'Changes', icon: GitCompareArrows }, { id: 'agents', label: 'Agents', icon: Bot }];
  let tabMenuOpen = $state(false);
  let tabList = $state<HTMLElement | null>(null);
  let tabOverflowStart = $state(false);
  let tabOverflowEnd = $state(false);
  ...
  function updateTabOverflow() {
    ...
    tabOverflowStart = tabList.scrollLeft > 1;
    tabOverflowEnd = tabList.scrollLeft < maxScroll - 1;
  }
  ...
  $effect(() => {
    ...
    void tick().then(updateTabOverflow);
    return () => observer.disconnect();
  });

  $effect(() => {
    const tabSignature = `${openTabs.join('|')}:${dockTab}`;
    void tabSignature;
    void tick().then(updateTabOverflow);
  });
  ...
  <section class="surface-chooser" aria-label="Right panel surfaces">
    <div class="surface-grid">
      {#each surfaceOptions as option (option.id)}
        <Button class="surface-card" variant="ghost" onclick={() => openSurface(option.id)}><option.icon /><span>{option.label}</span></Button>
      {/each}
      <Button class="surface-card" variant="ghost" onclick={onopenworktrees}><FolderGit2 /><span>Worktrees</span></Button>
    </div>
  </section>
{:else}
<Tabs.Root bind:value={dockTab} class="dock-root">
  <header class="dock-header">
    <Tabs.List bind:ref={tabList} variant="line" data-overflow-start={tabOverflowStart} data-overflow-end={tabOverflowEnd} onscroll={updateTabOverflow}>
  ...
  <Tabs.Content value="browser" class="dock-panel browser-panel">
    ...
    <div class="browser-body" data-browser-open={browserState.open}>
      {#if browserState.open}
        <div class="browser-native-host" bind:this={browserHost} aria-label="Embedded browser viewport"></div>
      {:else}
...
<style>
  ...
  .surface-grid { inline-size: 100%; max-inline-size: 17rem; display: flex; flex-wrap: wrap; justify-content: center; gap: .5rem; }
  :global(.surface-card) { inline-size: 8.25rem; aspect-ratio: 1.18; min-block-size: 5.5rem; display: grid; place-content: center; justify-items: center; gap: .65rem; border: 1px solid var(--border); border-radius: calc(var(--radius) * 1.2); background: transparent; padding: .8rem; color: var(--muted-foreground); text-align: center; white-space: normal; }
  ...
  :global(.surface-card) span { min-inline-size: 0; font-size: var(--type-small); }
  ...
  .dock-header :global([data-slot='tabs-list']) { --tab-edge-fade: 1.35rem; min-inline-size: 0; max-inline-size: 100%; overflow: auto hidden; overscroll-behavior-inline: contain; scrollbar-width: none; }
  .dock-header :global([data-slot='tabs-list'][data-overflow-end='true']) { -webkit-mask-image: linear-gradient(to right, black 0, black calc(100% - var(--tab-edge-fade)), transparent 100%); mask-image: linear-gradient(to right, black 0, black calc(100% - var(--tab-edge-fade)), transparent 100%); }
  .dock-header :global([data-slot='tabs-list'][data-overflow-start='true'][data-overflow-end='false']) { -webkit-mask-image: linear-gradient(to right, transparent 0, black var(--tab-edge-fade), black 100%); mask-image: linear-gradient(to right, transparent 0, black var(--tab-edge-fade), black 100%); }
  .dock-header :global([data-slot='tabs-list'][data-overflow-start='true'][data-overflow-end='true']) { -webkit-mask-image: linear-gradient(to right, transparent 0, black var(--tab-edge-fade), black calc(100% - var(--tab-edge-fade)), transparent 100%); mask-image: linear-gradient(to right, transparent 0, black var(--tab-edge-fade), black calc(100% - var(--tab-edge-fade)), transparent 100%); }
  .dock-header :global([data-slot='tabs-list']::-webkit-scrollbar) { display: none; }
  .dock-tab { min-inline-size: 0; display: inline-flex; flex: none; align-items: center; border-radius: calc(var(--radius) * .75); }
  ...
  .dock-error { grid-row: 2; display: flex; align-items: center; gap: .5rem; padding: .4rem .7rem; border-block-end: 1px solid color-mix(in oklab, var(--destructive), transparent 50%); color: var(--destructive); font-size: var(--type-caption); }
  ...
  :global(.browser-panel) { padding: 0; }
  :global(.browser-panel[data-state='active']) { display: grid; grid-template-rows: auto minmax(0, 1fr); overflow: hidden; }
  .browser-body, .browser-start, .browser-context, .browser-native-host { min-inline-size: 0; min-block-size: 0; }
  .browser-body, .browser-native-host { block-size: 100%; }
  .browser-start { block-size: 100%; display: grid; grid-template-rows: auto minmax(0, 1fr); overflow: hidden; }
  .browser-context { display: grid; overflow: auto; }
  .browser-context :global([data-slot='empty']) { min-block-size: 100%; }
  ...
  .browser-address { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; gap: .3rem; margin: 0; padding: .45rem .55rem; border-block-end: 1px solid var(--border); }
  .browser-address :global(input) { font-family: var(--font-mono); font-size: var(--type-caption); }
  .browser-nav { display: flex; align-items: center; gap: .1rem; }
  .browser-tools { display: flex; align-items: center; gap: .1rem; }
  .local-server-list { margin: .55rem .55rem 0; border: 1px solid var(--border); border-radius: calc(var(--radius) * 1.1); overflow: clip; background: color-mix(in oklab, var(--surface-raised), transparent 18%); }
  ...
  .local-server-list > header strong { font-size: var(--type-small); font-weight: 620; }
  ...
  .server-copy strong { overflow: hidden; font-size: var(--type-small); font-weight: 590; text-overflow: ellipsis; white-space: nowrap; }
  .server-copy small { color: var(--muted-foreground); font-size: var(--type-caption); }
  .server-online { inline-size: .55rem; block-size: .55rem; border-radius: 50%; background: var(--status-positive); }
  .local-server-list > p { margin: 0; padding: .65rem; color: var(--muted-foreground); font-size: var(--type-caption); line-height: 1.4; }
  ...
  .preview-setup { align-self: start; padding: .55rem; }
  ...
  .preview-launch strong, .preview-card strong { display: block; font-size: var(--type-small); }
  .preview-launch p, .preview-card span { display: block; max-inline-size: 52ch; margin: .16rem 0 0; overflow-wrap: anywhere; color: var(--muted-foreground); font-size: var(--type-caption); line-height: 1.45; }
  ...
  .file-toolbar strong { min-inline-size: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-family: var(--font-mono); font-size: var(--type-caption); }
  ...
  .file-list span { min-inline-size: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-family: var(--font-mono); font-size: var(--type-caption); }
  .file-list small { color: var(--muted-foreground); font-size: var(--type-caption); }
  ...
  .file-notice { color: var(--muted-foreground); font-size: var(--type-caption); }
  .file-toolbar kbd { margin-inline-start: .12rem; color: var(--muted-foreground); font-family: var(--font-mono); font-size: var(--type-caption); font-weight: 500; }
  ...
  @container surface-chooser (inline-size < 18rem) { .surface-grid { max-inline-size: 8.25rem; } }
