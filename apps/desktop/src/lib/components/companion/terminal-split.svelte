...
<section class="terminal-split" data-layout={layout} data-has-terminals={sessions.length > 0} aria-label="Worktree terminals">
  ...
  {#if layout === 'bottom' || sessions.length}
...
<style>
  ...
  .terminal-split[data-layout='sidebar'] { grid-template-columns: minmax(0, 1fr); grid-template-rows: auto auto minmax(0, 1fr); }
  .terminal-split[data-layout='sidebar'][data-has-terminals='true'] { grid-template-columns: minmax(0, 1fr) minmax(9rem, 32%); }
  ...
  .terminal-header { grid-row: 1; justify-content: space-between; gap: var(--density-gap); min-block-size: 2rem; padding: .3rem .55rem; border-block-end: 1px solid var(--border); color: var(--muted-foreground); font-size: var(--type-small); }
  ...
  .terminal-error { grid-row: 2; display: flex; align-items: center; gap: .4rem; border-block-end: 1px solid color-mix(in oklab, var(--destructive), transparent 50%); padding: .3rem .55rem; color: var(--destructive); font-size: var(--type-caption); } .terminal-error span { min-inline-size: 0; flex: 1; } .terminal-error :global(svg) { inline-size: .8rem; flex: none; }
  .terminal-output { grid-row: 3; inline-size: 100%; min-block-size: 0; resize: none; overflow: auto; border: 0; border-radius: 0; background: transparent; padding: .65rem .75rem; color: inherit; font-family: var(--font-mono); font-size: var(--type-small); line-height: 1.5; white-space: pre; scrollbar-gutter: stable; }
  .terminal-output:focus-visible { outline: 2px solid var(--ring); outline-offset: -2px; }
  .terminal-tabs { grid-row: 4; min-inline-size: 0; justify-content: space-between; gap: .35rem; border-block-start: 1px solid var(--border); background: var(--background); padding: .18rem .35rem .18rem .45rem; }
  ...
  .terminal-tab > button:first-child { overflow: hidden; border: 0; background: transparent; padding: .28rem .2rem .28rem .4rem; color: inherit; font-family: var(--font-mono); font-size: var(--type-caption); text-overflow: ellipsis; white-space: nowrap; } .terminal-tab > button:first-child > span { margin-inline-start: .3rem; color: var(--muted-foreground); font-size: var(--type-caption); }
  ...
  :global(.terminal-empty) { grid-row: 3; align-self: center; }
</style>
