...
<Dialog.Root bind:open>
    ...
    {#if worktrees.length}<details class="worktree-details"><summary>Manage existing worktrees ({worktrees.length})</summary><Table.Root><Table.Caption>Active coding-thread worktrees.</Table.Caption><Table.Header><Table.Row><Table.Head>Branch</Table.Head><Table.Head>Path</Table.Head><Table.Head>Actions</Table.Head></Table.Row></Table.Header><Table.Body>{#each worktrees as worktree (worktree.worktreeId)}<Table.Row><Table.Cell>{worktree.branch}</Table.Cell><Table.Cell><code>{worktree.path}</code></Table.Cell><Table.Cell><Button size="icon-xs" variant="ghost" disabled={pending === worktree.worktreeId} onclick={() => removeWorktree(worktree.worktreeId)} aria-label={`Remove worktree ${worktree.branch}`}><Trash2 /></Button></Table.Cell></Table.Row>{/each}</Table.Body></Table.Root></details>{/if}
  </Dialog.Content>
</Dialog.Root>
